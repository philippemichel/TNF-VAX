...
Une analyse factorielle en analyse de composante principale est tentée après discrétisation de l'âge. Les variables de nombre d'injections ont été supprimées car trop liées à la variable" `vaccination complète`.

```{r}
#| label: MCA

ttz <- tt |> 
  select(- c(ddn, pat.id,date.2e.inj, intro.tnf, nb.inj.tot,nb.inj.med)) |> 
...
Sueles les variables les plus influentes ont été gardée. La couleur représente la participation de la variable au graphique. Les variabes qui permettent le mieux de catégorisé les cas sont `"Aucune comorbidité"` & `"traitement immunosuppresseur, biothérapie (autre que anti TNF alpha), et/ou corticothérapie"`. le statut `"VIH"` semble aussi être important.

```{r}
#| label: MCAindvac
#| fig-cap: MCA - rerpésentation des individus seon l'état vaccinal

fviz_mca_ind(rtt, habillage = "vaccin.compl", addEllipses = TRUE, repel = TRUE)
```

La vaccination complète ne semble pas diffrentier clairement deux gropues de patients. 
